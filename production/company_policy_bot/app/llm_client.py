...
{raw_text}"""
    ...
    def summarize_audio_text(self, transcript: str) -> ChatResult:
        system_prompt = "당신은 최고 수준의 기업 회의록 전문 비서입니다. 주어진 녹취록을 공식 회의록으로 작성하세요."
        user_prompt = f"""다음 녹취록을 상세히 분석하여 아래 양식으로 매우 상세하게 작성하세요.
1. 🎯 [회의 개요]: 핵심 주제/목적 (2~3문장)
2. 🗣️ [주요 논의 사항]: 안건별로 최소 '3문장 이상' 구체적 기술 (고유명사, 숫자, 날짜, 금액 필수 포함)
3. ✅ [결정 사항]: 최종 합의된 내용
4. 🚀 [향후 실행 계획]: 구체적인 Action Items
